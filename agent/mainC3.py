...
class MyRob(CRobLinkAngs):    
           ...
           or self.measures.irSensor[back_id]   > 5.0:
            ...
            if self.finished:
                if (round(x), round(y))==(0,0):
                    self.create_path()
                    self.finish()
                self.move2start(self.state[5-int(round(x)/2)][12+int(round(y)/2)])
                return
            if (x,y)==(0,0) and self.measures.ground != -1:
                if self.measures.ground not in self.beacons:
                    self.beacons[self.measures.ground]=(0,0)
            ...
            if self.has_new_coords:
                ...
                if self.arrived:
                    ...
                    if self.measures.ground != -1:
                        if self.measures.ground not in self.beacons:
                            self.beacons[self.measures.ground]=(self.new_x, self.new_y)
                        if len(self.beacons) == self.nBeacons:
                            self.finished = True
                            self.create_path()
    ...
    def create_path(self):
        ...
        bcn = self.state[5][12]
        final_path = []
        while len(self.beacons!=0):
            pth = self.closest_beacon(bcn)
            final_path = final_path + pth
            bcn = pth[-1]
        final_path = final_path + self.path2start(bcn)

        # print
        with open('output.txt', 'w') as file:
            for cell in final_path:
                print(cell.coords[0], cell.coords[1], file=file)
            
        return
    
    def closest_beacon(self, curr_cell):
        visited_cells = set()
        visited_cells.add(curr_cell)
        queue = deque([(curr_cell,0,[])])
        ...
        while queue:
            ...
            for neighbour in neighbours:
                ...
                if neighbour not in visited_cells:
                    ...
                    new_parents.append(neighbour)
                    new_moves = moves
                    for key, value in self.beacons:
                        if value == neighbour.coords:
                            remove_key = key
                    if remove_key:
                        del self.beacons[remove_key]
                        remove_key=None
                        return new_parents
